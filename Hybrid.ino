...
Adafruit_MAX31855 Thermocouples [4] = {thermocouple1,thermocouple2,thermocouple3,thermocouple4}; //create thermocouple array
...
void loop() {
    ...
    for(int j = 0; j < 2; j++){
      ...
      TC[j] = Thermocouples[j].readCelsius(); //first 2 thermocouples are on run tank
      TC[j+2] = Thermocouples[j+2].readCelsius(); //second 2 thermocouples are on combustion chamber
    }
...
//interrupt that 
void Ignition(){
  while (digitalRead(ARMED)==HIGH){  //only capable of ignition if the safety lock on the engine has been disarmed
